...
def max_1(a, axis=None, keepdims=None, initial=None, where=None):
...
def test_max():
	...
	for a, axis, keepdims, initial, where in itertools.product(*[(A), (None, 1, -1, [0,-1]), (True, False), (None, 7), (None, whr)]):
		try:
			res1 = np.max(A, axis=axis, keepdims=keepdims, initial=initial, where=where)
		except:
			continue
		wherel = where.tolist() if where is not None else None
		res2 = np.asarray(max_1(A.tolist(), axis=axis, keepdims=keepdims, initial=initial, where=wherel))  
