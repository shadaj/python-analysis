...
def all_1(a, axis=None, keepdims=None, where=None):
	...
	output = util.trues(shape_output)
...
def test_all():
	...
	for a, axis, keepdims, where in itertools.product(*[(A), (None, 1, -1, [0,-1]), (True, False), (None, whr)]):
		res1 = np.all(A, axis=axis, keepdims=keepdims, where=where)
		wherel = where.tolist() if where is not None else None
		res2 = np.asarray(all_1(A.tolist(), axis=axis, keepdims=keepdims, where=wherel)) 
